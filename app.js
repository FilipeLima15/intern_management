...
function sampleData(){
  ...
  const users = [];
  // ALTERAÇÃO 1: Senha do admin principal MANTIDA EM BRANCO AQUI
  // Se for o primeiro acesso, o initApp() a definirá no Firebase com uma senha TEMPORÁRIA para segurança
  users.push({ id: uuid(), username: 'admin', name: 'Administrador Principal', password: '', role: 'super', powers: defaultPowersFor('super'), selfPasswordChange: true, createdAt: now });
...
async function initApp(){
  ...
  if ((state.users || []).length === 0 && (state.interns || []).length === 0) {
    console.log("Banco vazio — inicializando com sampleData em memória. Criando Admin Principal com senha temporária.");
    
    // Configura o sampleData. Agora, a senha do admin principal é definida AQUI
    state = sampleData();
    const adminUser = state.users.find(u => u.username === 'admin' && u.role === 'super');
    if (adminUser) {
        // Senha temporária, DEVE SER MUDADA pelo Admin Principal
        adminUser.password = 'default_init_pass_12345';
        console.warn("ADMIN PRINCIPAL CRIADO COM SENHA TEMPORÁRIA: default_init_pass_12345. Mude a senha imediatamente!");
    }
    
    // Grava o sample data com a senha TEMPORÁRIA no Firebase
    await save(state);
  }
...
// initApp(); // Removido para ser chamado no final
